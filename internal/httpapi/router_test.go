...
func TestSessionTradingAccountEndpointReturnsSelectedModeSnapshot(t *testing.T) {
	...
	if storedSession.TradingAccount != nil {
		t.Fatalf("expected refresh endpoint to avoid persisting trading account snapshot")
	}
	if storedSession.TradingUpdatedAt != nil {
		t.Fatalf("expected refresh endpoint to avoid persisting trading updated_at")
	}
}
...
func TestSessionTradingAccountEndpointFallsBackToStoredSnapshotOnRefreshError(t *testing.T) {
	...
	if got := payload["trading_updated_at"]; got == nil {
		t.Fatalf("expected trading_updated_at to be present")
	}
}

func TestSessionTradingAccountEndpointDoesNotReuseStoredSnapshotFromDifferentMode(t *testing.T) {
	st := store.NewMemoryStore()
	storedAccount := model.TradingAccountSnapshot{
		Mode:           "paper",
		Status:         "ACTIVE",
	...
	if got := payload["trading_account"]; got != nil {
		t.Fatalf("expected no fallback trading account for mismatched mode, got %v", got)
	}
	if got := payload["trading_updated_at"]; got != nil {
		t.Fatalf("expected no trading_updated_at for mismatched mode fallback, got %v", got)
	}
}
